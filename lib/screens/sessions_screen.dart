...
class _SessionsScreenState extends State<SessionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Your Lessons Sessions'),
      ),
      body: ReorderableListView(
        ...
        onReorder: (oldIndex, newIndex) {
          setState(() {
            ...
            final element = tiles.removeAt(oldIndex);
            tiles.insert(newIndex, element);

            final element1 = sessions.removeAt(oldIndex);
            sessions.insert(newIndex, element1);
          });
        }),
  ...
  Future saveSession() async {
    ...
    helper.writSession(newSession).then((_){
      updateScreen();
      helper.setCounter();
    });
    txtDescription.text = '';
